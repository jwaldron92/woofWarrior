...
if(isset($_POST['id']) && isset($_POST['name'])){
	...
	if($user['userFBID'] == NULL && $user['email'] == NULL){
		...
		mysqli_query($link, "UPDATE `user` SET `userFBID`=".$_POST['id'].", `userFBNAME`='".$_POST['name']."' WHERE `email`='".$_POST['email']."';") or die(mysqli_error($link));
	}elseif($user['email'] != $_POST['email'] && $user['userFBID'] == $_POST['id']){
		mysqli_query($link, "UPDATE `user` SET `email`='".$_POST['email']."', `userFBNAME`='".$_POST['name']."' WHERE `userFBID`='".$_POST['id']."';") or die(mysqli_error($link));
	}
	...
	if($_POST['id'] == '1661737827403858'){
		$_SESSION['username'] =  'Woof Warrior';
		mysqli_query($link, "UPDATE `user` SET `username`= 'Woof Warrior' WHERE `userFBID`='".$_POST['id']."';") or die(mysqli_error($link));
	}else{
...
if(isset($_POST['email']) && isset($_POST['password'])){
	...
	$stmt = mysqli_prepare($link, "SELECT `email`, `password`, `active` FROM `user` WHERE `email`=?;") or die(mysqli_error($link));
	...
	if(!empty($user)){
		if($user['active'] == '0'){
		    ...
		    if(strcmp(crypt($_POST['password'], $user['password']), $user['password']) === 0){
