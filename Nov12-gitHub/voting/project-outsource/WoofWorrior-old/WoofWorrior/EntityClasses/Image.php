...
class Image
{
   ...
   public function __construct()
   {
     ...
     * @return int|array
     */
   public function upload($path, $user, $description)
   {
       ...
       if($this->result)
       {
           ...
           {
               $this->query = "INSERT INTO image(UserID, Description, Location) VALUES ($user, '$description', '$path')";
   ...
   public function getImages()
   {
       ...
       if($this->result && (mysqli_num_rows($this->result) > 0)) //check query result and number of affected rows
       {
           ...
           while( ($row = mysqli_fetch_assoc($this->result)) !=false )
           {
               ...
               array_push($temp, [ "ID"=>$imageID, "Description"=>$description, "Path"=>$location  ]);
           }
      ...
      if($this->result)
      {
        ...
        if(mysqli_num_rows($this->result) > 0)
        {
            while (($row = mysqli_fetch_assoc($this->result)) != false)
            {
                ...
                $location = $temp[$indexer]['Location'];
                if (array_key_exists("Vote", $row)) {
                    $vote = $row['Vote'];
                }
                array_push($this->data, ["ID" => $imageID, "Description" => $description, "Path" => $location, "Vote" => $vote]);
                $indexer += 1;
            }
        }
        else
   ...
   private function add_Row_Votes_Table($imageID, $userID)
   {
       ...
       //prepare query
       $this->query = "INSERT INTO vote (UserID, ImageID) VALUES ($imageID , $userID)";
       $this->result = $this->db->Select($this->query);
       if($this->result)
       {
           //query success
