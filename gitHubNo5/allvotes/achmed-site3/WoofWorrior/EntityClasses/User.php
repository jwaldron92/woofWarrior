        ...
        if ($this->result) {
            ...
            if ($this->result) // get the result of the query
            {
                ...
                if ($count > 0)
                {
                    ...
                    session_write_close();
                    array_push($this->status, [ "Status"=>"ok", "ID"=>$user['UserID'], "Name"=> $username ]);
                }
                else
                {
                    $array =  $this->createUser($username);
                    return $array;
                }
            }
            else
        ...
        if($this->result)
        {
            ...
            if($this->result) // get the result of the query
            {
                ...
                if($count > 0)
                {
                    ...
                    {
                        //User exists as well as the directory now create the session and write array
                        array_push($this->status, [ "Status"=>"ok"]);
                        $user = mysqli_fetch_assoc($this->result);
